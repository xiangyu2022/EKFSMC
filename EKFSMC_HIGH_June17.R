...
data_gen=function(pop, I0, S0=pop-I0, N, t, dist_mat, d, lambda, reco_rate, 
  ...
  x_S[,1] = S0
  x_R[,1] = pop - I0 - S0
  #Kd = d * dnorm(1, mean = dist_mat, sd = d)
  Kd = d * dnorm(dist_mat, mean = 0, sd = d)
...
ekf_smc=function(dat, m, resampling, lambda, dist_mat, d, SCORE=TRUE, reco_rate,
  ...
  y = dat$y
  N = dat$N
  S = Imat = array(NA, c(N, m, t+1))
  lgW = lgW_lag = matrix(NA, nrow=m, ncol=t+1)
  ...
  Kd = Kd2 = d * dnorm(dist_mat, mean = 0, sd = d)
  ...
  for (j in 1:t) {
    ...
    indx = is.na(y[,j+1])
    K_t = lambda * sigma_cond[ , ,j+1]/(lambda^2 * sigma_cond[ , ,j+1] + sigma_y^2) 
    K_t[indx,] = y[indx,j+1] = 0
    mu_I[ , ,j+1] = mu_cond_I[ , ,j+1] + K_t * (y[ ,j+1] - lambda*mu_cond_I[ , ,j+1])
    sigma[ , ,j+1] = sigma_cond[ , ,j+1] * (1 - lambda * K_t)
    
    Imat[ , ,j+1] = mu_I[ , ,j+1] + matrix(rnorm(N*m, 0, sigma_I), nrow=N, ncol=m)
    S[ , ,j+1] = S[ , ,j] + (1-reco_rate)*Imat[ , ,j] - Imat[ , ,j+1]
    for (b in 1:m){
      lgW[b,j+1] = dmvnorm(Imat[ , b,j+1], 
                           mean=(1 - reco_rate + trans_rate_inside * S[ , b,j])*Imat[ , b,j] + trans_rate_outside * S[ , b,j] * KdImat[, b],
                           sigma=diag(sigma_I^2), log=T) + 
        dmvnorm(y[,j+1], mean=lambda*Imat[ , b,j+1], sigma=diag(sigma_y^2), log=T) -
        dmvnorm(Imat[ , b,j+1], mean=mu_I[ , b,j+1], sigma=diag(sigma_I^2), log=T) + lgW[ b,j]
    }
    ...
    if(resampling[j]==1){
      ww=exp(lgW[ ,j+1])
      ww_prob=ww/sum(ww)
      r_index=resampling.res(mm=m, ww=ww_prob)
      Imat[, ,j+1]=Imat[ ,r_index,j+1]
      S[, ,j+1]=S[, r_index,j+1]
      lgW[ ,j+1]=0
      loglike =loglike+log(mean(ww))
    }
    else if(resampling[j]==2){
      ww=exp(lgW[ ,j+1])
      ww_prob=ww/sum(ww)
      r_index=resampling.str(mm=m, ww=ww_prob)
      Imat[ , ,j+1]=Imat[ ,r_index,j+1];S[, ,j+1]=S[,r_index,j+1]
      lgW[ ,j+1]=0
      loglike =loglike + log(mean(ww))
    }
  ...
  return(list(S=S, Imat=Imat, lgW=lgW, lgW_lag=lgW_lag, loglike=loglike,lambda=lambda))
}

#EKF-SMC
set.seed(12)
trans_rate_inside=dat$trans_rate_inside*1
trans_rate_outside=dat$trans_rate_outside*1
reco_rate=dat$reco_rate*1
resu_plpo=ekf_smc(dat=dat, m=m, 
                  resampling=rep(rep(c(0,1,0,2),c(1,1,1,1)), 10),
                  lambda=0.9, dist_mat=dist_mat, 
                  d=2,
                  SCORE=FALSE, 
                  reco_rate=reco_rate,
                  trans_rate_inside = trans_rate_inside, 
                  trans_rate_outside = trans_rate_outside, 
                  sigma_I=dat$sigma_I, sigma_y=dat$sigma_y)

par(mar=c(0.5,0.5,0.5,0.5))
par(mfrow=c(3,3))
l=2 # This specifies which region you want to generate plots
for (tobs in 3:40){
  if (tobs%%8==2){
  ...
  w_obs=exp(resu_plpo$lgW_lag[,tobs+1]) 
