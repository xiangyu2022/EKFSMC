...
data_gen=function(pop, I0, S0=pop-I0, N, t, reco_rate, trans_rate, sigma_I, sigma_y){
  ...
  return(list(y=y, x_I=x_I, x_S=x_S, x_R=x_R, 
              reco_rate=reco_rate, trans_rate=trans_rate, 
              sigma_I=sigma_I, sigma_y=sigma_y, t=t, N=N, pop=pop))
}

ekf_smc=function(dat, m, lambda, resam_sdl, SCORE=TRUE, seed=NULL, 
  ...
  for (j in 1:t) {
    ...
    indx = is.na(y[,j+1])
    K_t = sigma_cond[ , ,j+1]/(sigma_cond[ , ,j+1] + sigy_h^2) # For convenience below
    K_t[indx,]=0
    mu_I[ , ,j+1] = mu_cond_I[ , ,j+1] + K_t * (y[ ,j+1] - mu_cond_I[ , ,j+1])
    sigma[ , ,j+1] = sigma_cond[ , ,j+1] * (1 - K_t)

    # So far, we assume all the different regions have a similar obeservation variance 
    Imat[ , ,j+1] = mu_I[ , ,j+1] + matrix(rnorm(N*m, 0, sigI_h), nrow=N, ncol=m)
    S[ , ,j+1] = S[ , ,j] + (1-reco_rate)*Imat[ , ,j] - Imat[ , ,j+1] 
    lgW[ , ,j+1] = dnorm(Imat[ , ,j+1], mean=(1 - reco_rate + trans_rate * S[ , ,j])*Imat[ , ,j], sd=sigI_h, log=T)+dnorm(y[,j+1], mean=Imat[ , ,j+1], sd=sigy_h, log=T)-dnorm(Imat[ , ,j+1], mean=mu_I[ , ,j+1], sd=sigI_h, log=T)+lgW[ , ,j]
    ...
    if(SCORE!=0){
      if(SCORE==2){ # 2016
        for (l in 1:N){
          wtm1<-exp(lgW[l, ,j])
          grad_f<-cbind(S[l, ,j]*Imat[l, ,j]*(Imat[l, ,j+1] - (1 - reco_rate[l] + trans_rate[l] * S[l, ,j])* Imat[l, ,j]), - Imat[l, ,j]*(Imat[l, ,j+1] - (1 - reco_rate[l] + trans_rate[l] * S[l, ,j])* Imat[l, ,j]))/sigI_h^2
